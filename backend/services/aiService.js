...
const generateImageResponse = async (imageBuffer, mimeType, language = "english", retries = 2) => {
...
"This explanation is for informational purposes only and should not replace advice from a qualified lawyer."`;
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      console.error(`Gemini Vision API Error (attempt ${attempt + 1}):`, error?.message || error);
      if (attempt === retries) {
        throw new Error(error?.message || "AI service temporarily unavailable. Please try again.");
      }
      await new Promise(r => setTimeout(r, 1000 * (attempt + 1)));
